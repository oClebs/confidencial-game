...
const JanelaExterna = ({ children, onClose }) => {
  ...
  useEffect(() => {
    ...
    win.document.title = "DOSSIÊ SECRETO - SAFE MODE";
...
function App() {
  ...
  useEffect(() => {
    ...
    return () => { 
      socket.off('sala_criada_sucesso'); socket.off('entrada_sucesso'); socket.off('erro_login'); socket.off('atualizar_sala'); socket.off('sala_encerrada'); 
      socket.off('aviso_sala'); socket.off('inicio_preparacao'); socket.off('nova_rodada'); socket.off('fase_decifrar'); socket.off('sincronizar_tempo'); 
      socket.off('resultado_rodada'); socket.off('fim_de_jogo'); socket.off('sessao_invalida'); socket.off('banido_da_sala'); socket.off('log_evento');
  ...
  const copiarLinkConvite = () => {
      const url = `${window.location.origin}?room=${sala}`;
  ...
  const commonRender = (content) => ( <> <AvisoToast /> <SystemLogs /> {entrou && (<button onClick={sairDaSala} style={exitBtnStyle}>ABANDONAR</button>)} {entrou && (<button onClick={() => setModoStreamerLocal(!modoStreamerLocal)} style={streamerBtnStyle} title="Modo Streamer (Ocultar Segredos)">{modoStreamerLocal ? '🙈' : '👁️'}</button>)} {menuBan.visivel && (<div style={menuBanStyle} onClick={confirmarBan}>🔨 BANIR AGENTE <br/> <span style={{color: 'white'}}>{menuBan.jogadorNome}</span></div>)} {entrou && fase !== 'LOBBY' && fase !== 'FIM' && <SidebarJogadores />} {content} </> );
        ...
        <div style={{ border: '4px solid #333', padding: '40px', background: '#f4e4bc', color: '#333', width: '100%', maxWidth: '500px', boxSizing: 'border-box', boxShadow: '10px 10px 0 rgba(0,0,0,0.5)', position: 'relative' }}>
          ...
          {modoLogin === 'CRIAR' && (
            <div style={{ marginTop: '20px' }}>
              ...
              <div style={{ background: '#e8dcb5', padding: '15px', border: '2px solid #333', margin: '10px 0', textAlign: 'left' }}><h4 style={{ margin: '0 0 10px 0', borderBottom: '1px solid #333' }}>⚙️ AJUSTES DO SISTEMA</h4><label style={{ display: 'flex', alignItems: 'center', margin: '5px 0', cursor: 'pointer', color: '#000', fontWeight: 'bold' }}><input type="checkbox" checked={configSala.twitchAuth} onChange={e => setConfigSala({...configSala, twitchAuth: e.target.checked})} style={{ marginRight: '10px' }} /> <span style={{ color: '#9146FF' }}>👾 Exigir Autenticação Twitch (Host Login)</span></label><label style={{ display: 'flex', alignItems: 'center', margin: '5px 0', cursor: 'pointer', color: '#000', fontWeight: 'bold' }}><input type="checkbox" checked={configSala.streamerMode} onChange={e => setConfigSala({...configSala, streamerMode: e.target.checked})} style={{ marginRight: '10px' }} />Modo Streamer (Janela Segura)</label><div style={{ margin: '10px 0' }}><label style={{ fontSize: '14px', fontWeight: 'bold' }}>CICLOS DE RODADAS (Voltas na mesa):</label><div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginTop: '5px' }}><input type="range" min="1" max="5" value={configSala.numCiclos} onChange={e => setConfigSala({...configSala, numCiclos: parseInt(e.target.value)})} style={{ flex: 1, cursor: 'pointer' }} /><span style={{ fontWeight: 'bold', fontSize: '18px', width: '30px' }}>{configSala.numCiclos}</span></div></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '10px' }}><div><label style={{ fontSize: '12px', fontWeight: 'bold' }}>PREPARAÇÃO (s):</label><input type="number" value={configSala.tempos.preparacao} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, preparacao: e.target.value}})} style={{ ...inputStyle, padding: '5px' }} /></div><div><label style={{ fontSize: '12px', fontWeight: 'bold' }}>SABOTAGEM (s):</label><input type="number" value={configSala.tempos.sabotagem} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, sabotagem: e.target.value}})} style={{ ...inputStyle, padding: '5px' }} /></div><div style={{ gridColumn: 'span 2' }}><label style={{ fontSize: '12px', fontWeight: 'bold' }}>DECIFRAÇÃO (s):</label><input type="number" value={configSala.tempos.decifracao} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, decifracao: e.target.value}})} style={{ ...inputStyle, padding: '5px' }} /></div></div></div>
              {erroLogin && <p style={{color: '#b91c1c', fontWeight: 'bold', textAlign: 'center'}}>{erroLogin}</p>}
              <button onClick={acaoCriarSala} style={{ ...btnStyle, background: configSala.twitchAuth ? '#9146FF' : '#b91c1c' }}>{configSala.twitchAuth ? "LOGAR COM TWITCH & CRIAR" : "CRIAR SALA"}</button>
            ...
            <div style={{ marginTop: '20px' }}>
                ...
                {salaEhTwitch ? (
                    <div style={{ margin: '15px 0' }}>
                        ...
                        <button onClick={acaoEntrarSala} style={{ ...btnStyle, background: '#9146FF' }}>
