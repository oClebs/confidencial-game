...
function App() {
  // --- ESTILOS ORIGINAIS (MOVIDOS PARA CIMA PARA NÃO DAR ERRO) ---
        ...
        <div style={{ border: '4px solid #333', padding: '40px', background: '#f4e4bc', color: '#333', width: '100%', maxWidth: '500px', boxSizing: 'border-box', boxShadow: '10px 10px 0 rgba(0,0,0,0.5)', position: 'relative' }}>
          ...
          {modoLogin === 'CRIAR' && (
            <div style={{ marginTop: '20px' }}>
              ...
              {/* --- ⚙️ CONFIGURAÇÕES QUE ESTAVAM FALTANDO --- */}
          ...
          {modoLogin === 'ENTRAR' && (<div style={{ marginTop: '20px' }}><h3 style={{textAlign: 'center'}}>// LOGIN DE AGENTE //</h3><input placeholder="CODINOME" onChange={e => setNome(e.target.value)} style={inputStyle} /><input placeholder="CÓDIGO DA OPERAÇÃO (ID)" value={sala} onChange={e => setSala(e.target.value)} style={{...inputStyle, textTransform: 'uppercase'}} />{salaEhTwitch ? (<div style={{ margin: '15px 0' }}><p style={{ color: '#9146FF', fontWeight: 'bold', textAlign: 'center', marginBottom: '10px' }}>👾 SALA REQUER AUTENTICAÇÃO TWITCH</p><button onClick={acaoEntrarSala} style={{ ...btnStyle, background: '#9146FF' }}>LOGAR COM TWITCH & ENTRAR</button></div>) : (<><input placeholder="SENHA DE ACESSO (Opcional se for Twitch)" type="text" onChange={e => setSenha(e.target.value)} style={inputStyle} /><button onClick={acaoEntrarSala} style={{ ...btnStyle, background: '#15803d' }}>ACESSAR</button></>)}{erroLogin && <p style={{color: '#b91c1c', fontWeight: 'bold', textAlign: 'center'}}>{erroLogin}</p>}<button onClick={() => {setModoLogin('MENU'); setErroLogin('');}} style={{ ...btnStyle, background: 'transparent', color: '#333', border: '2px solid #333' }}>CANCELAR</button></div>)}
        </div>
      </div>
    );
  }
  ...
  // (Blocos de fases PREPARACAO, SABOTAGEM, ETC... continuam iguais aos anteriores)
  // Vou colocar apenas o fechamento do App.jsx para você ter o arquivo completo funcional.
