...
function App() {
  ...
  // LISTA DE PALAVRAS TENTADAS NA SABOTAGEM
  ...
  useEffect(() => {
    ...
    socket.on('sincronizar_tempo', ({ duracao, fim }) => { const agora = Date.now(); const restante = Math.max(0, Math.ceil((fim - agora) / 1000)); setTempoRestante(restante); });
  ...
  const acaoCriarSala = () => { 
      const senhaValida = configSala.twitchAuth ? true : (senha && senha.length > 0);
  ...
  const mainWrapper = {
    ...
    minHeight: '100vh', width: '100%', fontFamily: "'Courier New', Courier, monospace", color: '#e5e5e5', boxSizing: 'border-box', padding: '20px'
  };
  ...
  const paperStyle = { 
    backgroundColor: '#f4e4bc', backgroundImage: 'linear-gradient(#e8dcb5 1px, transparent 1px)', backgroundSize: '100% 1.5em', 
    color: '#1a1a1a', padding: '40px', boxShadow: '5px 5px 15px rgba(0,0,0,0.5)', maxWidth: '800px', width: '100%', 
    margin: '30px auto', border: '1px solid #cfb997', fontSize: '22px', lineHeight: '1.5em', textAlign: 'left', 
  ...
  const stickyNoteStyle = {
    backgroundColor: '#fef3c7', color: '#333', padding: '15px 30px', display: 'inline-block', transform: 'rotate(2deg)',
    boxShadow: '3px 3px 8px rgba(0,0,0,0.4)', marginTop: '20px', fontFamily: "'Courier New', Courier, monospace", 
    textAlign: 'center', border: '1px solid #eab308'
  };

  const stampStyle = {
    border: '3px solid', padding: '5px 10px', textTransform: 'uppercase', fontWeight: 'bold', fontSize: '14px', 
    transform: 'rotate(-10deg)', opacity: 0.9, marginTop: 'auto', letterSpacing: '1px', textAlign: 'center', fontFamily: 'sans-serif'
  };

  const rulesBtnStyle = {
    position: 'fixed', bottom: '20px', right: '20px', width: '60px', height: '60px', borderRadius: '50%',
    backgroundColor: '#d97706', color: '#fff', fontSize: '30px', fontWeight: 'bold', border: '3px solid #fff',
    cursor: 'pointer', boxShadow: '0 4px 10px rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center'
  };

  const exitBtnStyle = {
    position: 'fixed', top: '20px', left: '20px',
    backgroundColor: 'transparent', color: '#ef4444', border: '2px solid #ef4444',
    padding: '10px 15px', fontWeight: 'bold', cursor: 'pointer', zIndex: 2000,
    fontFamily: 'monospace', textTransform: 'uppercase'
  };

  const rulesBoxStyle = {
    position: 'fixed', bottom: '90px', right: '20px', width: '300px', backgroundColor: '#f4e4bc',
    color: '#1c1917', padding: '20px', border: '4px solid #1c1917', borderRadius: '4px',
    boxShadow: '-5px 5px 15px rgba(0,0,0,0.7)', zIndex: 1999, fontFamily: "'Courier New', Courier, monospace",
    transform: exibirRegras ? 'scale(1)' : 'scale(0)', transformOrigin: 'bottom right', transition: 'transform 0.2s ease-out'
  };

  const menuBanStyle = {
    position: 'fixed', top: menuBan.y, left: menuBan.x, backgroundColor: '#1c1917', color: '#ef4444',
    border: '2px solid #ef4444', padding: '10px 20px', zIndex: 99999, cursor: 'pointer',
    boxShadow: '0 4px 10px rgba(0,0,0,0.5)', fontWeight: 'bold', fontSize: '14px'
  };
  ...
  const SidebarJogadores = () => (
      <div style={{
          ...
          width: '80px', background: '#1c1917', border: '2px solid #333',
          padding: '10px 5px', borderRadius: '10px', zIndex: 500,
          boxShadow: '0 0 10px rgba(0,0,0,0.5)',
          display: 'flex', flexDirection: 'column', gap: '15px', alignItems: 'center'
      }}>
          <div style={{ fontSize: '10px', color: '#666', fontWeight: 'bold', textAlign: 'center', borderBottom: '1px solid #333', width: '100%', paddingBottom: '5px' }}>
              AGENTS
          </div>
          {jogadores.map(j => (
              <div 
                ...
                title={souHost && j.id !== socket.id ? "Botão Direito para BANIR" : j.nome}
                style={{
                    width: '50px', height: '50px', background: j.id === socket.id ? '#d97706' : '#e2e8f0',
                    borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    border: j.isHost ? '3px solid #b91c1c' : '2px solid #333',
                    fontSize: '24px', cursor: souHost ? 'context-menu' : 'default',
                    position: 'relative'
                }}
              >
                  🕵️‍♂️
                  <span style={{ position: 'absolute', bottom: '-15px', fontSize: '10px', color: '#fff', background: '#000', padding: '0 4px', borderRadius: '4px', whiteSpace: 'nowrap' }}>
                      {j.nome.substring(0,6)}
                  </span>
              </div>
          ))}
      </div>
      ...
      <div style={{
          ...
          {logsSistema.map(log => (
              <div key={log.id} style={{
                  backgroundColor: 'rgba(0,0,0,0.8)', borderLeft: `4px solid ${log.tipo === 'ban' ? '#ef4444' : (log.tipo === 'entrada' ? '#22c55e' : '#eab308')}`,
                  color: '#fff', padding: '10px 15px', borderRadius: '4px', fontFamily: 'monospace', fontSize: '14px',
  ...
  const commonRender = (content) => (
      <>
        ...
        <SystemLogs />
        {entrou && (
            <button onClick={sairDaSala} style={exitBtnStyle}>
                ABANDONAR
            </button>
        )}
        ...
        <div style={{ border: '4px solid #333', padding: '40px', paddingTop: '80px', background: '#f4e4bc', color: '#333', width: '100%', maxWidth: '500px', boxSizing: 'border-box', boxShadow: '10px 10px 0 rgba(0,0,0,0.5)', position: 'relative' }}>
          ...
          {modoLogin === 'CRIAR' && (
            <div style={{ marginTop: '20px' }}>
              ...
              <input 
                placeholder={configSala.twitchAuth ? "🔒 AUTENTICAÇÃO TWITCH ATIVA" : "DEFINIR SENHA DE ACESSO"} 
                type="text" 
                disabled={configSala.twitchAuth}
                onChange={e => setSenha(e.target.value)} 
                value={configSala.twitchAuth ? "" : senha}
                style={{ ...inputStyle, opacity: configSala.twitchAuth ? 0.6 : 1, cursor: configSala.twitchAuth ? 'not-allowed' : 'text' }} 
              />
              ...
              <div style={{ background: '#e8dcb5', padding: '15px', border: '2px solid #333', margin: '10px 0', textAlign: 'left' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', margin: '5px 0', cursor: 'pointer', color: '#000', fontWeight: 'bold' }}>
                      ...
                      Exigir Autenticação Twitch (Substitui Senha)
                  </label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '10px' }}>
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: 'bold' }}>PREPARAÇÃO (s):</label>
                        <input type="number" value={configSala.tempos.preparacao} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, preparacao: e.target.value}})} style={{ ...inputStyle, padding: '5px' }} />
                      </div>
                      <div>
                        <label style={{ fontSize: '12px', fontWeight: 'bold' }}>SABOTAGEM (s):</label>
                        <input type="number" value={configSala.tempos.sabotagem} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, sabotagem: e.target.value}})} style={{ ...inputStyle, padding: '5px' }} />
                      </div>
                      <div style={{ gridColumn: 'span 2' }}>
                        <label style={{ fontSize: '12px', fontWeight: 'bold' }}>DECIFRAÇÃO (s):</label>
                        <input type="number" value={configSala.tempos.decifracao} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, decifracao: e.target.value}})} style={{ ...inputStyle, padding: '5px' }} />
                      </div>
                  </div>
              </div>
      ...
      <div style={mainWrapper}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center', width: '100%', maxWidth: '1000px' }}>{jogadores.map((j, i) => (
            <div 
                key={j.id} 
                onContextMenu={(e) => handleContextMenuJogador(e, j)}
                title={souHost && j.id !== socket.id ? "Botão Direito para BANIR" : ""}
                style={{ ...agentCardStyle, transform: `rotate(${i % 2 === 0 ? '2deg' : '-2deg'})`, cursor: souHost ? 'context-menu' : 'default' }}
            >
                <div style={{ width: '70px', height: '70px', background: '#e2e8f0', borderRadius: '50%', marginBottom: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid #333' }}><span style={{ fontSize: '35px' }}>🕵️‍♂️</span></div>
  ...
  // OUTRAS FASES
  if (fase === 'PREPARACAO') {
      const devoEsconder = souHost && configRecebida?.streamerMode;
      ...
      <div style={{ ...mainWrapper, background: '#2c3e50' }}>
          ...
          {meuPapel === 'SABOTADOR' && (
              <div style={{ 
                  ...
                  padding: '20px', borderTop: '2px dashed #666', width: '100%', maxWidth: '800px'
              }}>
                  ...
                  {palavrasSabotadasRodada.map((p, i) => (
                      <div key={i} style={{
                          backgroundColor: '#fef08a', color: '#000', padding: '5px 15px', 
