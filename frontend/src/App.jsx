...
const styles = {
  ...
  monitorBezel: {
    ...
    borderRadius: '20px',
    boxShadow:
      '0 0 0 4px #060806, inset 0 0 28px rgba(0,0,0,0.9)',
  },
  ...
  consoleBottom: {
    ...
    padding: '10px 25px', zIndex: 0
  },
  consoleKnob: { width: '30px', height: '30px', borderRadius: '50%', background: 'radial-gradient(circle at 30% 20%, #fdfdfd 0%, #aaa 40%, #111 100%)', boxShadow: '0 4px 8px rgba(0,0,0,0.9)' },
  consoleLamp: { width: '14px', height: '22px', borderRadius: '8px', background: 'radial-gradient(circle at 50% 0%, #ffe89a 0%, #fbbf24 30%, #7c2d12 80%)', boxShadow: '0 0 12px rgba(251, 191, 36, 0.9)' },
  ...
  paper: {
    ...
    padding: '30px',
    boxShadow: '0 10px 30px rgba(0,0,0,0.8), inset 0 0 40px rgba(160, 140, 100, 0.2)',
    maxWidth: '100%',
    width: '100%',
  ...
  folderTab: {
    ...
    fontSize: '12px', fontWeight: 'bold', color: '#8a7a60', letterSpacing: '1px',
    boxShadow: 'inset 0 10px 10px -10px rgba(255,255,255,0.8)',
  },

  // Inputs e Botões
  inputCRT: {
    padding: '10px 12px', margin: '8px 0', fontSize: '16px',
    color: '#d5ffd9', backgroundColor: 'rgba(0, 20, 5, 0.6)',
    border: '1px solid rgba(156, 253, 177, 0.4)',
    width: '100%', fontFamily: "'Courier Prime', monospace", fontWeight: 600,
    boxSizing: 'border-box', outline: 'none', textTransform: 'uppercase', letterSpacing: '1px'
  },
  ...
  btnCRT: {
    ...
    boxShadow: '0 0 10px rgba(0,255,150,0.1)', transition: 'all 0.1s'
  },

  // Cards de Jogadores
  agentCard: {
    backgroundColor: '#fff', color: '#111', padding: '10px 10px 25px 10px',
    width: '110px', margin: '10px',
    boxShadow: '3px 3px 8px rgba(0,0,0,0.6)',
    display: 'flex', flexDirection: 'column', alignItems: 'center',
    transform: 'rotate(2deg)', border: '1px solid #ccc', fontSize: '12px'
  },

  // Botões Flutuantes (Sair, Streamer)
  navBar: {
    display: 'flex', justifyContent: 'space-between', marginBottom: '20px',
    borderBottom: '1px solid rgba(0,255,150,0.3)', paddingBottom: '10px'
  }
};
  ...
  <div style={styles.mainWrapper}>
    <div style={styles.monitorWrapper}>
      ...
      <div style={styles.consoleBottom}>
        ...
        <div style={styles.consoleKnob} />
        <div style={{width:'60px', height:'20px', background:'#333', borderRadius:'4px', border:'1px solid #555'}}></div>
...
function App() {
  ...
  const TopBar = () => (
    <div style={styles.navBar}>
        ...
        <div style={{display:'flex', gap:'15px'}}>
            <button onClick={sairDaSala} style={{background:'transparent', border:'none', color:'#ffb3b3', cursor:'pointer', fontSize:'12px'}}>SAIR [X]</button>
            <button onClick={() => setModoStreamerLocal(!modoStreamerLocal)} style={{background:'transparent', border:'none', color:'#afffbf', cursor:'pointer', fontSize:'12px'}}>
                {modoStreamerLocal ? 'MODO STREAMER: ON' : 'MODO STREAMER: OFF'}
            </button>
        </div>
    </div>
  );
  ...
  const Timer = () => (
    <div style={{position:'absolute', top:'10px', right:'15px', color:(tempoRestante < 10 ? '#ffb3b3' : '#e5f5d0'), fontSize:'18px', fontWeight:'bold', textShadow: '0 0 5px currentColor'}}>
  ...
  const renderContent = () => {
    // 1. TELA DE LOGIN (Sem papel, direto no terminal verde)
    if (!entrou) {
      ...
      return (
        <div style={{textAlign:'center', height:'100%', display:'flex', flexDirection:'column', justifyContent:'center'}}>
            <img src={logoImage} style={{width:'50%', maxWidth:'300px', margin:'0 auto 20px auto', filter:'drop-shadow(0 0 5px rgba(0,0,0,0.5))'}} />
            ...
            {emMenu && (
                <div style={{display:'flex', flexDirection:'column', gap:'10px', alignItems:'center'}}>
                    ...
                    <button onClick={() => setModoLogin('ENTRAR')} style={{...styles.btnCRT, background:'transparent', border:'1px solid #6ed88b'}}>ACESSAR OPERAÇÃO EXISTENTE</button>
                </div>
            )}
            ...
            {emCriar && (
                <div style={{width:'100%', maxWidth:'400px', margin:'0 auto'}}>
                    ...
                    <div style={{textAlign:'left', margin:'10px 0', fontSize:'12px', color:'#9ae79f'}}>
                    ...
                    <button onClick={() => setModoLogin('MENU')} style={{background:'transparent', border:'none', color:'#9ae79f', marginTop:'10px', cursor:'pointer'}}>VOLTAR</button>
                </div>
            )}
            ...
            {emEntrar && (
                <div style={{width:'100%', maxWidth:'400px', margin:'0 auto'}}>
                    ...
                    {!salaEhTwitch && (
                        <>
                            <input placeholder="SEU CODINOME" onChange={e=>setNome(e.target.value)} style={styles.inputCRT}/>
                            <input placeholder="SENHA DA SALA" type="text" onChange={e=>setSenha(e.target.value)} style={styles.inputCRT}/>
                        </>
                    )}

                    {salaEhTwitch && <p style={{color:'#bd93f9', fontSize:'12px'}}>🔒 ESTA SALA REQUER LOGIN TWITCH</p>}

                    <button onClick={acaoEntrarSala} style={{...styles.btnCRT, background: salaEhTwitch ? '#bd93f9' : styles.btnCRT.backgroundColor, color: salaEhTwitch ? '#000' : '#e5ffe0'}}>
                        {salaEhTwitch ? 'LOGAR COM TWITCH' : 'ENTRAR NA SALA'}
                    </button>
                    <button onClick={() => setModoLogin('MENU')} style={{background:'transparent', border:'none', color:'#9ae79f', marginTop:'10px', cursor:'pointer'}}>VOLTAR</button>
                </div>
            )}
    ...
    // 2. LOBBY (Pasta dentro do monitor)
    if (fase === 'LOBBY') {
        return (
            <div>
                <TopBar/>
                <div style={{textAlign:'center', marginBottom:'20px'}}>
                    <div style={styles.paper}>
                        ...
                        <h2 style={{margin:'0', color:'#b91c1c', fontSize:'32px', letterSpacing:'3px'}}>CÓDIGO: {sala}</h2>
                        <p style={{fontSize:'12px', color:'#555', margin:'5px 0'}}>// PESSOAL AUTORIZADO APENAS //</p>
                        <button onClick={copiarLinkConvite} style={{border:'1px dashed #333', background:'transparent', cursor:'pointer', padding:'5px 10px', fontSize:'11px'}}>
                            {linkCopiado ? 'COPIADO!' : 'COPIAR LINK DE CONVITE'}
                        </button>
                    </div>
                </div>
                ...
                <div style={{display:'flex', flexWrap:'wrap', gap:'15px', justifyContent:'center'}}>
                    {jogadores.map(j => (
                        <div key={j.id} onContextMenu={e=>handleContextMenuJogador(e,j)} style={styles.agentCard}>
                            <div style={{width:'60px', height:'60px', background:'#eee', marginBottom:'5px', overflow:'hidden', border:'1px solid #999'}}>
                                {j.foto ? <img src={j.foto} style={{width:'100%', height:'100%'}}/> : <span style={{fontSize:'30px', lineHeight:'60px'}}>🕵️</span>}
                            </div>
                            <strong style={{fontSize:'11px', textTransform:'uppercase'}}>{j.nome}</strong>
                            <span style={{fontSize:'10px', color:'#666'}}>{j.pontos} PTS</span>
                            {j.isHost && <span style={{color:'red', fontWeight:'bold', fontSize:'10px'}}>DIRETOR</span>}
                        </div>
                    ))}
                </div>
                ...
                <div style={{textAlign:'center', marginTop:'30px'}}>
                    {souHost ? (
                        <button onClick={iniciarJogo} style={{...styles.btnCRT, background:'#eab308', color:'#000', border:'none'}}>INICIAR OPERAÇÃO</button>
                    ) : (
                        <p style={{color:'#afffbf', animation:'pulse 2s infinite'}}>AGUARDANDO O DIRETOR INICIAR...</p>
                    )}
                </div>
            </div>
        );
        ...
        return (
            <div>
                ...
                {!jaEnvieiPreparacao ? (
                    <>
                        {devoEsconder ? (
                            <div style={{border:'2px dashed #afffbf', padding:'20px', textAlign:'center', color:'#afffbf', marginTop:'50px'}}>
                                ...
                                <button onClick={() => setJanelaExternaAberta(true)} style={styles.btnCRT}>ABRIR PAINEL</button>
                            </div>
                        ) : (
                            <>
                                <div style={{background:'#000', color:'#eab308', padding:'10px', textAlign:'center', border:'1px solid #eab308', marginBottom:'20px'}}>
                                    PALAVRA SECRETA: <strong style={{fontSize:'24px'}}>{minhaPalavraInicial}</strong>
                                </div>
                                <div style={styles.paper}>
                                    <textarea rows={6} autoFocus placeholder="Descreva a palavra..." value={textoPreparacao} onChange={e=>setTextoPreparacao(e.target.value)} style={{...styles.inputPaper, background:'transparent', border:'none', resize:'none', fontFamily:'"Courier Prime", monospace'}}/>
                                </div>
                    ...
                    <div style={{textAlign:'center', marginTop:'100px', color:'#afffbf'}}>
                        <h2>RELATÓRIO ENVIADO</h2>
                        <p>Aguardando outros agentes ({statusPreparacao.prontos}/{statusPreparacao.total})...</p>
                    </div>
                )}
            </div>
        );
    ...
    if (fase === 'SABOTAGEM') {
        return (
            <div>
                <TopBar/><Timer/>
                <div style={{borderBottom:'1px solid #333', marginBottom:'10px', paddingBottom:'5px', fontSize:'12px', color:'#aaa'}}>
                    ARQUIVO: {meuPapel} | RODADA {infoRodada.atual}/{infoRodada.total}
                </div>

                {meuPapel === 'DECIFRADOR' && (
                    <div style={{textAlign:'center', marginTop:'80px', color:'#ffb3b3'}}>
                        <h1 style={{fontSize:'40px', border:'4px solid #ffb3b3', display:'inline-block', padding:'10px'}}>ACESSO NEGADO</h1>
                ...
                {meuPapel === 'CIFRADOR' && (
                    <div style={styles.paper}>
                        ...
                        <h3 style={{color:'#b91c1c', marginTop:0}}>SEU TEXTO ESTÁ SENDO ATACADO:</h3>
                        <p style={{fontSize:'18px'}}>"{descricaoRecebida}"</p>
                    </div>
                )}

                {meuPapel === 'SABOTADOR' && (
                    <>
                        <div style={{textAlign:'center', background:'#111', padding:'10px', border:'1px solid #eab308', color:'#eab308', marginBottom:'15px'}}>
                            ALVO: <strong style={{fontSize:'20px'}}>{dadosRodada?.palavra}</strong>
                        </div>
                        {!sabotagemEnviada ? (
                            <div style={styles.paper}>
                                <div style={styles.folderTab}>CENSURA //</div>
                                {inputsSabotagem.map((v, i) => (
                                    <input key={i} placeholder={`PALAVRA PROIBIDA #${i+1}`} value={v} onChange={e=>atualizarInputSabotagem(i,e.target.value)} style={styles.inputPaper} />
                                ))}
                                <button onClick={enviarSabotagem} style={{...styles.btnCRT, background:'#b91c1c', border:'none'}}>EXECUTAR CENSURA</button>
                            </div>
                        ) : (
                            <div style={{textAlign:'center', marginTop:'50px', color:'#afffbf'}}>
    ...
    if (fase === 'DECIFRANDO') {
        return (
            <div>
                <TopBar/><Timer/>
                <div style={{textAlign:'center', marginBottom:'20px', color:'#afffbf'}}><h2>DECODIFICAÇÃO</h2></div>
                
                <div style={styles.paper}>
                    <div style={{position:'absolute', top:'10px', right:'10px', border:'2px solid black', padding:'2px 5px', fontSize:'10px', transform:'rotate(-10deg)', opacity:0.5}}>CLASSIFIED</div>
                    <p style={{fontSize:'20px', lineHeight:'1.6'}}>
                        {textoCensurado.split(/(\[CENSURADO\])/g).map((parte, i) => (
                            parte === '[CENSURADO]' 
                            ? <span key={i} style={{background:'#111', color:'transparent', padding:'0 5px'}}>████</span> 
                ...
                {meuPapel === 'SABOTADOR' && (
                    <div style={{marginTop:'20px', textAlign:'center'}}>
                        <p style={{fontSize:'12px', color:'#aaa'}}>TENTATIVAS DA EQUIPE:</p>
                        <div style={{display:'flex', flexWrap:'wrap', gap:'5px', justifyContent:'center'}}>
                            {palavrasSabotadasRodada.map((p, i) => (
                                <span key={i} style={{background:'#eab308', color:'#000', padding:'2px 6px', fontSize:'11px', borderRadius:'2px'}}>
                ...
                {meuPapel === 'DECIFRADOR' ? (
                    <div style={{marginTop:'20px'}}>
                        <input placeholder="QUAL É A PALAVRA?" value={tentativaDecifrador} onChange={e=>setTentativaDecifrador(e.target.value)} style={styles.inputCRT} autoFocus/>
                        <button onClick={enviarDecifracao} style={{...styles.btnCRT, background:'#2563eb', border:'none'}}>ENVIAR RESPOSTA</button>
                    </div>
                ) : (
                    <p style={{textAlign:'center', color:'#aaa', marginTop:'20px'}}>// AGUARDANDO ANÁLISE DO DECIFRADOR //</p>
                )}
            </div>
        );
    }
    ...
    if (fase === 'RESULTADO' && resultadoRodada) {
        return (
            <div style={{textAlign:'center'}}>
                <TopBar/>
                <h1 style={{color: resultadoRodada.acertou ? '#afffbf' : '#ffb3b3', textShadow: '0 0 10px currentColor'}}>
                    {resultadoRodada.acertou ? 'SUCESSO NA DECIFRAÇÃO' : 'FALHA NA DECIFRAÇÃO'}
                </h1>
                
                <div style={styles.paper}>
                    <div style={styles.folderTab}>RELATÓRIO //</div>
                    <p>A PALAVRA ERA: <strong style={{color:'#b91c1c', fontSize:'24px'}}>{resultadoRodada.palavraSecreta}</strong></p>
                    <p>O DECIFRADOR DISSE: <strong>{resultadoRodada.tentativa}</strong></p>
                    <hr style={{borderColor:'#aaa'}}/>
                    <ul style={{textAlign:'left', fontSize:'14px'}}>
                        {resultadoRodada.resumo.map((l, i) => <li key={i} style={{marginBottom:'5px'}}>{l}</li>)}
                    </ul>
                </div>

                {souHost ? (
                    <button onClick={proximaRodada} style={styles.btnCRT}>PRÓXIMA RODADA ➡️</button>
                ) : (
    ...
    if (fase === 'FIM') {
        return (
            <div style={{textAlign:'center'}}>
                ...
                <div style={{border:'2px solid #afffbf', padding:'20px', maxWidth:'400px', margin:'0 auto'}}>
                    {jogadores.map((j, i) => (
                        <div key={j.id} style={{display:'flex', justifyContent:'space-between', borderBottom:'1px dashed #afffbf', padding:'5px 0', color:'#afffbf'}}>
