...
const styles = {
  ...
  btn: { padding: '20px', background: '#333', color: 'white', border: 'none', marginTop: '10px', fontSize: '1.2rem', fontFamily: 'monospace', fontWeight: 'bold', cursor: 'pointer', width: '100%', textTransform: 'uppercase' },
  paper: { backgroundColor: '#f4e4bc', backgroundImage: 'linear-gradient(#e8dcb5 1px, transparent 1px)', backgroundSize: '100% 1.5em', color: '#1a1a1a', padding: '40px', boxShadow: '5px 5px 15px rgba(0,0,0,0.5)', maxWidth: '800px', width: '100%', margin: '30px auto', border: '1px solid #cfb997', fontSize: '22px', lineHeight: '1.5em', textAlign: 'left', position: 'relative', transform: 'rotate(-1deg)' },
  agentCard: { backgroundColor: '#fff', color: '#333', padding: '20px 15px', width: '180px', height: '240px', boxShadow: '0 4px 10px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', fontFamily: "'Courier New', Courier, monospace", transition: 'transform 0.2s', border: '1px solid #ccc' },
  stickyNote: { backgroundColor: '#fef3c7', color: '#333', padding: '15px 30px', display: 'inline-block', transform: 'rotate(2deg)', boxShadow: '3px 3px 8px rgba(0,0,0,0.4)', marginTop: '20px', fontFamily: "'Courier New', Courier, monospace'", textAlign: 'center', border: '1px solid #eab308' },
  stamp: { border: '3px solid', padding: '5px 10px', textTransform: 'uppercase', fontWeight: 'bold', fontSize: '14px', transform: 'rotate(-10deg)', opacity: 0.9, marginTop: 'auto', letterSpacing: '1px', textAlign: 'center', fontFamily: 'sans-serif' },
  rulesBtn: { position: 'fixed', bottom: '20px', right: '20px', width: '60px', height: '60px', borderRadius: '50%', backgroundColor: '#d97706', color: '#fff', fontSize: '30px', fontWeight: 'bold', border: '3px solid #fff', cursor: 'pointer', boxShadow: '0 4px 10px rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  streamerBtn: { position: 'fixed', top: '20px', right: '20px', width: '50px', height: '50px', borderRadius: '50%', backgroundColor: '#333', color: '#fff', fontSize: '24px', border: '2px solid #fff', cursor: 'pointer', boxShadow: '0 4px 10px rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  exitBtn: { position: 'fixed', top: '20px', left: '20px', backgroundColor: 'transparent', color: '#ef4444', border: '2px solid #ef4444', padding: '10px 15px', fontWeight: 'bold', cursor: 'pointer', zIndex: 2000, fontFamily: 'monospace', textTransform: 'uppercase' },
  rulesBox: { position: 'fixed', bottom: '90px', right: '20px', width: '300px', backgroundColor: '#f4e4bc', color: '#1c1917', padding: '20px', border: '4px solid #1c1917', borderRadius: '4px', boxShadow: '-5px 5px 15px rgba(0,0,0,0.7)', zIndex: 1999, fontFamily: "'Courier New', Courier, monospace", transformOrigin: 'bottom right', transition: 'transform 0.2s ease-out' },
  menuBan: { position: 'fixed', backgroundColor: '#1c1917', color: '#ef4444', border: '2px solid #ef4444', padding: '10px 20px', zIndex: 99999, cursor: 'pointer', boxShadow: '0 4px 10px rgba(0,0,0,0.5)', fontWeight: 'bold', fontSize: '14px' }
};
    ...
    return (
      <div style={styles.mainWrapper}>
        ...
        <div style={{ border: '4px solid #333', padding: '40px', background: '#f4e4bc', color: '#333', width: '100%', maxWidth: '500px', boxSizing: 'border-box', boxShadow: '10px 10px 0 rgba(0,0,0,0.5)', position: 'relative' }}>
          ...
          {modoLogin === 'MENU' && (<div style={{ marginTop: '20px' }}><button onClick={() => setModoLogin('CRIAR')} style={styles.btn}>INICIAR NOVA OPERAÇÃO</button><button onClick={() => setModoLogin('ENTRAR')} style={{...styles.btn, background: '#57534e'}}>ACESSAR OPERAÇÃO EXISTENTE</button></div>)}
              ...
              <div style={{ background: '#e8dcb5', padding: '15px', border: '2px solid #333', margin: '10px 0', textAlign: 'left' }}>
                  ...
                  <div style={{ margin: '10px 0' }}>
                      ...
                      <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginTop: '5px' }}>
                          <input type="range" min="1" max="5" value={configSala.numCiclos} onChange={e => setConfigSala({...configSala, numCiclos: parseInt(e.target.value)})} style={{ flex: 1, cursor: 'pointer' }} />
                          <span style={{ fontWeight: 'bold', fontSize: '18px', width: '30px' }}>{configSala.numCiclos}</span>
                      </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '10px' }}>
                      <div><label style={{ fontSize: '12px', fontWeight: 'bold' }}>PREPARAÇÃO (s):</label><input type="number" value={configSala.tempos.preparacao} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, preparacao: e.target.value}})} style={{ ...styles.input, padding: '5px' }} /></div>
                      <div><label style={{ fontSize: '12px', fontWeight: 'bold' }}>SABOTAGEM (s):</label><input type="number" value={configSala.tempos.sabotagem} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, sabotagem: e.target.value}})} style={{ ...styles.input, padding: '5px' }} /></div>
                      <div style={{ gridColumn: 'span 2' }}><label style={{ fontSize: '12px', fontWeight: 'bold' }}>DECIFRAÇÃO (s):</label><input type="number" value={configSala.tempos.decifracao} onChange={e => setConfigSala({...configSala, tempos: {...configSala.tempos, decifracao: e.target.value}})} style={{ ...styles.input, padding: '5px' }} /></div>
                  </div>
              </div>

              {erroLogin && <p style={{color: '#b91c1c', fontWeight: 'bold', textAlign: 'center'}}>{erroLogin}</p>}
              <button onClick={acaoCriarSala} style={{ ...styles.btn, background: configSala.twitchAuth ? '#9146FF' : '#b91c1c' }}>
    ...
    return commonRender(
      <div style={styles.mainWrapper}>
        ...
        <div style={{ borderBottom: '4px solid #d97706', paddingBottom: '20px', marginBottom: '30px', textAlign: 'center', width: '100%' }}>
          ...
          <div style={styles.stickyNote}><span style={{ display: 'block', fontSize: '10px', fontWeight: 'bold', marginBottom: '5px' }}>CÓDIGO DA MISSÃO:</span><strong style={{ fontSize: '2.5rem', letterSpacing: '3px' }}>{sala}</strong></div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '30px', justifyContent: 'center', width: '100%', maxWidth: '1000px' }}>{jogadores.map((j, i) => (<div key={j.id} onContextMenu={(e) => handleContextMenuJogador(e, j)} title={souHost && j.id !== socket.id ? "Botão Direito para BANIR" : ""} style={{ ...styles.agentCard, transform: `rotate(${i % 2 === 0 ? '2deg' : '-2deg'})`, cursor: souHost ? 'context-menu' : 'default' }}><div style={{ width: '70px', height: '70px', background: '#e2e8f0', borderRadius: '50%', marginBottom: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid #333', overflow: 'hidden' }}>{j.foto ? <img src={j.foto} alt="Avatar" style={{width: '100%', height: '100%', objectFit: 'cover'}} /> : <span style={{fontSize: '35px'}}>🕵️‍♂️</span>}</div><div style={{ fontSize: '1.3rem', fontWeight: 'bold', textTransform: 'uppercase', borderBottom: '2px solid #333', width: '100%', textAlign: 'center', paddingBottom: '5px', marginBottom: '5px' }}>{j.nome}</div><div style={{ fontSize: '0.9rem', color: '#666', fontFamily: 'sans-serif' }}>SCORE: {j.pontos}</div><div style={{ marginTop: 'auto', marginBottom: '10px' }}>{j.isHost ? (<div style={{ ...styles.stamp, borderColor: '#b91c1c', color: '#b91c1c' }}>MISSION DIRECTOR</div>) : (<div style={{ ...styles.stamp, borderColor: '#15803d', color: '#15803d', transform: 'rotate(-5deg)' }}>FIELD AGENT</div>)}</div></div>))}</div>
